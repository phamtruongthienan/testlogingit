...
<head>
    ...
    <link rel="stylesheet" href="{{asset('assets/frontend/libs/font-awesome/css/all.min.css')}}">
...
@foreach($ads as $ka => $va)
    ...
    @if($va->position == 1 && $va->type==2)
        ...
        <div class="popup_this" style="display:none">
            <a href="{{ $ads_link }}" target="_blank"><img src="{{$ads_image}}" width="500px" alt="Ads Esearch"></a>
        </div>
        @break;
    @endif
@endforeach
...
<script async src="https://www.googletagmanager.com/gtag/js?id={{$config_main[0]->configMaintranslations[0]->analytics_id}}"></script>
<script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());
    gtag('config', '{{$config_main[0]->configMaintranslations[0]->analytics_id}}');
</script>
