<nav role="navigation" class="nav-menu-2 w-nav-menu">
    ...
    <div class="div-block">
        @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
            @if(LaravelLocalization::getCurrentLocale() != $properties['regional'])
                <a class="lead margin-right-10 nav-language" id="switch-language" rel="alternate" hreflang="{{ $localeCode }}"
    ...
    <div class="div-block-72-23-copy-copy">
        ...
        <div class="div-block-74"><a href="#" data-w-id="8d4f4dc9-53e4-97fc-4148-7cebf3adfe86"
                                     class="button-6-copy-copy w-button btn-search-advanced-header">Advanced search</a></div>
    </div>
</nav>
@foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
    @if(LaravelLocalization::getCurrentLocale() != $properties['regional'])
        <a class="lead nav-language-copy" id="switch-language" rel="alternate" hreflang="{{ $localeCode }}"
           href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}">
            <span style="font-size:20px" class="flag-icon flag-icon-{{ $properties['regional'] }}"></span>
        </a>
    @endif
