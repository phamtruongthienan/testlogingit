        ...
        @if(count($promotion) > 0)
            ...
            <div class="div-block-1288 block-promotion-list">
                ...
                @foreach($promotion as $k => $v)
                    @php
                        $logo_promotion =  Imgfly::imgPublic($v->mSchooleventtranslations[0]->logo.'?w=300&h=93&fit=crop-center');
                    @endphp
                    @if($class_promotion == 0)
                        <div class="div-promotion" style="background-image: url({{$logo_promotion}});">
                            <a href="{{asset($v->mSchooleventtranslations[0]->slug)}}"
                               class="link-promotion w-inline-block">
                               <div class="logo-background-3" style="float:left">{{$v->mSchooleventtranslations[0]->name}}</div>
                            </a>
                        </div>
                    @else
