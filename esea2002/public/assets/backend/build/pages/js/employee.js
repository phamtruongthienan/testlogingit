...
app.init = function () {
			...
			success: function (response) {
					if (response.code == '200') {	
						...
						$.each(response.data.activity, function(i, item) {
							a += '<tr><td>'+item.id+'</td><td>'+item.created_at+'</td><td>'+item.causer_type+'</td><td>'+item.description+'</td><td>'+JSON.parse(item.properties).status+'</td></tr>';
						});
						$('#activity').html(a);
	...
	$(document).on('click', '#addEmployee', function (e) {
		...
		$('#modal-employee #ttlModal').html(" Thêm nhân viên mới");
		$('#modal-employee').modal('show');		
	});
