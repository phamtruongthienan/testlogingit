...
app.init = function () {
	...
	$('#sendForm').validator().on('submit', function (e) {
		...
		if (!e.isDefaultPrevented()) {
				e.preventDefault();
				app.submitForm1(table_dynamic_email);
				$('#send-mail-form').modal('hide');
	...
	app.submitForm1 = function (table_dynamic_email) {
		$.ajax({
			url: base_admin + "/ajax/emailStatus",
			data: $('#sendForm').serialize(),
			type: "post",
			success: function (response) {
					if (response.code == '200') {	
						table_dynamic_email.ajax.reload(null, true);
						$('#alert_msg_edit').removeClass('no-display');
						Lobibox.notify("success", {
							title: 'Thông báo',
							pauseDelayOnHover: false,
	...
	$(document).on('click', '.mail-status-delete', function (e) {
		e.preventDefault();
		$('#send-mail-form #id').val($(this).data('id'));
        app.clearForm('delete');
		var row = $(this);
		$('#confirm-delete-modal').modal({
				backdrop: 'static',
	...
	$(document).on('click', '.table-action-edit', function (e) {
		...
		$('#modal-account-edit #inputEditPassWord').prop('required',false);
	...
	$(document).on('click', '#addAccount', function (e) {
        ...
        $('#sendForm').validator('validate');
		$('#modal-account-edit #inputEditSMTP').prop('required',true);
		$('#modal-account-edit #inputEditPassWord').prop('required',true);
		$('#modal-account-edit #inputEditrePassWord').prop('required',true);
		$('#modal-account-edit #inputEditPort').prop('required',true);
		$('#modal-account-edit #inputEditUserName').prop('required',true);
		$('#modal-account-edit #inputEditProtocal').prop('required',true);
		$('#modal-account-edit #inputEditSender').prop('required',true);
	...
	$(document).on('change', '#inputAddType', function (e) {
		var id= $(this).val();
		$.get("ajax/groupmail/"+id,function(data){
			$("#add_email").html(data);
		});
	});
	...
	$(document).on('click', '#send-mail', function (e) {
		e.preventDefault();
		$('#send-mail-form #action').val('insert');
		$('#sendForm').submit();
	});
...
app.clearForm = function (action) {
	...
	$('#send-mail-form #action').val(action);
