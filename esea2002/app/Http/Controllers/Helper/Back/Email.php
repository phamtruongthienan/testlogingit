...
class Email extends Controller
{
    ...
    public function getDTEmailStatus()
    {
        ...
        try {
            ...
            return Datatables::of($data)
            ->editColumn('action', function ($v){
                return ' <a class="table-action text-red table-action-delete1 mail-status-delete cursor-pointer" data-id="' . $v->id . '"><i' .
                ' class="fa fa-trash"></i></a>';
            })
        ...
        try {
            ...
            if($request->action == 'insert') {
                ...
                for($i = 0;$i < count($email); $i++){
                    ...
                    if($request->has('inputAddTitle') && !empty($request->inputAddTitle)) {
                        $data[$i]['title'] = $request->inputAddTitle;
                    }
        
                    if($request->has('inputAddContent') && !empty($request->inputAddContent)) {
                        $data[$i]['content'] = $request->inputAddContent;
                    }
        
                    if($request->has('inputAddType') && !empty($request->inputAddType)) {
                        $data[$i]['group_id'] = $request->inputAddType;
                    }
                    $smpt_id = ConfigEmail::where('default',1)->pluck('id')->first();
                    $data[$i]['smtp_id'] = $smpt_id;
                }
            }
            
            if($request->action == 'insert') {
                if(count($data) > 0) {
                    $query = MEmail::insert($data);
                    if(!$query) {
            ...
            if ($query) {
                DB::commit();
                if($request->action == 'insert') {
                    dispatch(new \App\Jobs\EmailSendMail($email,$request->inputAddTitle, $request->inputAddContent));
                }
                return true;
            } else {
                DB::rollback();
