...
use App\Models\MSchoolLevelTranslation;
...
class Level extends Controller
{
    ...
    //DEMO
    public function postLevel($request)
    {
        self::__construct();
        try {
            DB::beginTransaction();
            if($request->action == 'update' || $request->action == 'delete') {
                $query = MSchoolLevel::find($request->id);
                if(!$query) {
                    DB::rollback();
                    return false;
                }
            ...
            if($request->action == 'update') {
                if (count($data_relationship) > 0) {
                    $query->mSchoolLevelTranslationsAll()->where('language_id', $request->lang)->update($data_relationship);
                    if (!$query) {
                        DB::rollback();
                        return false;
                    }
                ...
                $ref = MSchoolLevelTranslation::where('translation_id', $request->id);
                $ref = $ref->delete();
                if(!$ref) {
                    DB::rollback();
                    return false;
                ...
                $query = MSchoolLevel::create($data);
                if(!$query) {
                    DB::rollback();
                    return false;
                }
                ...
                $data_relationship['translation_id'] = $query->id;
								$trans = self::renderTrans($query->mSchoolLevelTranslations(), $data_relationship);
                if(!$trans) {
                    DB::rollback();
                    return false;
                }
    ...
    public function getLevel($id, $language = 1)
    {
        self::__construct();
        try {
            $data = MSchoolLevel::with('mSchoolLevelTranslationsAll')
                    ->where('id', $id)
                    ->whereHas('mSchoolLevelTranslationsAll', function ($query) use ($language) {
    ...
    public function getDTLevel()
    {
        self::__construct();
        try {
                $data = MSchoolLevel::with('mSchoolLevelTranslations');
                return Datatables::of($data)
								->editColumn('name', function ($v) {
									if(!empty($v->mSchoolLevelTranslations[0]->name)) {
										return $v->mSchoolLevelTranslations[0]->name;
									} else {
										return '';
									}
								})
