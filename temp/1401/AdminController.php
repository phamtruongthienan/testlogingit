...
class AdminController extends Controller
{
	...
	// 		'inputEditContent' => 'required|min:1|max:500',
	...
	{
		try {
			$type = MSchoolType::with('mSchoolTypeTranslations')->get();
			$category = MSchoolCategory::with('mSchoolCategoryTranslations')->get();
			return view('theme.backend.page.attribute')->with(['category' => $category,
			'type' =>$type
			]);
		} catch (\Exception $e) {
			return redirect()->guest(route('admin.error'));
	...
	public function admin_post_room_ajax(Request $request)
	{
		$rules = array(
			'inputAddName' => 'required|min:1|max:255',
			'inputAddPosition' => 'required|min:1|max:255',
			'inputAddNameOption' => 'min:1|max:255',
			'inputAddValueOption' => 'min:1|max:255',
			'action' => 'required|in:insert,update,delete',
		);
		if($request->action == 'update') {
			$rules['id'] = 'required|digits_between:1,10';
	...
	public function admin_attribute_group_ajax(Request $request)
	{
		$instance = self::instance(\App\Http\Controllers\Helper\Back\AttributeGroup::class);
		if($request->has('id') && !empty($request->id)) {
	...
	public function admin_post_attribute_group_ajax(Request $request)
	{
		$rules = array(
			'inputAddName' => 'required|min:1|max:255',
			'action' => 'required|in:insert,update,delete',
